async function post(json, endpoint) {
    const url = `http://localhost:5019/api${endpoint}`;
    const options = {
        method: 'POST',
        headers: {
        ...
        try {
            ...
            if (token == "" || token == "" || token == null || token == undefined) {
                ...
                if (roles.includes("Administrator")) {
                    ...
                    localStorage.setItem("rol", "index");
                }
...
async function register(email, user, password) {
    const url = "http://localhost:5019/api/Auth/register";
    let update = {
        email: email,
        username: user,
        password: password
...
async function login(user, password) {
    const url = "http://localhost:5019/api/Auth/login";
    let update = {
        username: user,
        password: password,
